    ...
    extends AbstractDevkitBasedDefinitionParser
{
    ...
    private BeanDefinitionBuilder getBeanDefinitionBuilder(ParserContext parserContext) {
        try {
            ...
            logger.error(("Cannot launch the mule app, the @Processor [create-comment] within the connector [kindling] is not supported in mule "+ muleVersion));
            throw new BeanDefinitionParsingException(new Problem(("Cannot launch the mule app, the @Processor [create-comment] within the connector [kindling] is not supported in mule "+ muleVersion), new Location(parserContext.getReaderContext().getResource()), null, noClassDefFoundError));
        }
    }

    public BeanDefinition parse(Element element, ParserContext parserContext) {
        BeanDefinitionBuilder builder = getBeanDefinitionBuilder(parserContext);
        builder.addConstructorArgValue("createComment");
        builder.setScope(BeanDefinition.SCOPE_PROTOTYPE);
        if (!hasAttribute(element, "config-ref")) {
            throw new BeanDefinitionParsingException(new Problem("It seems that the config-ref for @Processor [create-comment] within the connector [kindling] is null or missing. Please, fill the value with the correct global element.", new Location(parserContext.getReaderContext().getResource()), null));
        }
        parseConfigRef(element, builder);
        if (!parseObjectRef(element, builder, "comment", "comment")) {
            BeanDefinitionBuilder commentBuilder = BeanDefinitionBuilder.rootBeanDefinition(KindlingCommentExpressionHolder.class.getName());
            Element commentChildElement = DomUtils.getChildElementByTagName(element, "comment");
            if (commentChildElement!= null) {
                parseProperty(commentBuilder, commentChildElement, "className", "className");
                parseProperty(commentBuilder, commentChildElement, "id", "id");
                if (!parseObjectRef(commentChildElement, commentBuilder, "owner", "owner")) {
                    BeanDefinitionBuilder _ownerBuilder = BeanDefinitionBuilder.rootBeanDefinition(KindlingUserExpressionHolder.class.getName());
                    Element _ownerChildElement = DomUtils.getChildElementByTagName(commentChildElement, "owner");
                    if (_ownerChildElement!= null) {
                        ...
                        commentBuilder.addPropertyValue("owner", _ownerBuilder.getBeanDefinition());
                    }
                }
                parseProperty(commentBuilder, commentChildElement, "parentType", "parentType");
                if (hasAttribute(commentChildElement, "parent-ref")) {
                    if (commentChildElement.getAttribute("parent-ref").startsWith("#")) {
                        commentBuilder.addPropertyValue("parent", commentChildElement.getAttribute("parent-ref"));
                    } else {
                        commentBuilder.addPropertyValue("parent", (("#[registry:"+ commentChildElement.getAttribute("parent-ref"))+"]"));
                    }
                }
                if (hasAttribute(commentChildElement, "matchOptionIndex-ref")) {
                    if (commentChildElement.getAttribute("matchOptionIndex-ref").startsWith("#")) {
                        commentBuilder.addPropertyValue("matchOptionIndex", commentChildElement.getAttribute("matchOptionIndex-ref"));
                    } else {
                        commentBuilder.addPropertyValue("matchOptionIndex", (("#[registry:"+ commentChildElement.getAttribute("matchOptionIndex-ref"))+"]"));
                    }
                }
                parseProperty(commentBuilder, commentChildElement, "type", "type");
                parseProperty(commentBuilder, commentChildElement, "description", "description");
                parseProperty(commentBuilder, commentChildElement, "dateCreated", "dateCreated");
                parseProperty(commentBuilder, commentChildElement, "dateCreatedLocalized", "dateCreatedLocalized");
                parseProperty(commentBuilder, commentChildElement, "stateId", "stateId");
                parseProperty(commentBuilder, commentChildElement, "stateName", "stateName");
                parseProperty(commentBuilder, commentChildElement, "resourceUri", "resourceUri");
                parseListAndSetProperty(commentChildElement, commentBuilder, "attachments", "attachments", "attachment", new ParseDelegate<String>() {
