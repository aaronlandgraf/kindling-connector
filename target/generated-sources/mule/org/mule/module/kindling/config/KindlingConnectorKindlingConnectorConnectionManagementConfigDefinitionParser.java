...
import org.mule.module.kindling.connectivity.KindlingConnectorConfigConnectionManagementConnectionManager;
...
public class KindlingConnectorKindlingConnectorConnectionManagementConfigDefinitionParser
    extends AbstractDevkitBasedDefinitionParser
{

    private static Logger logger = LoggerFactory.getLogger(KindlingConnectorKindlingConnectorConnectionManagementConfigDefinitionParser.class);

    public BeanDefinition parse(Element element, ParserContext parserContext) {
        parseConfigName(element);
        BeanDefinitionBuilder builder = getBeanDefinitionBuilder(parserContext);
        builder.setScope(BeanDefinition.SCOPE_SINGLETON);
        setInitMethodIfNeeded(builder, KindlingConnectorConfigConnectionManagementConnectionManager.class);
        setDestroyMethodIfNeeded(builder, KindlingConnectorConfigConnectionManagementConnectionManager.class);
        parseProperty(builder, element, "username", "username");
        parseProperty(builder, element, "password", "password");
        parseProperty(builder, element, "companyName", "companyName");
        parseProperty(builder, element, "impersonationToken", "impersonationToken");
    ...
    private BeanDefinitionBuilder getBeanDefinitionBuilder(ParserContext parserContext) {
        try {
            return BeanDefinitionBuilder.rootBeanDefinition(KindlingConnectorConfigConnectionManagementConnectionManager.class.getName());
            ...
            logger.error(("Cannot launch the mule app, the configuration [config] within the connector [kindling] is not supported in mule "+ muleVersion));
            throw new BeanDefinitionParsingException(new Problem(("Cannot launch the mule app, the configuration [config] within the connector [kindling] is not supported in mule "+ muleVersion), new Location(parserContext.getReaderContext().getResource()), null, noClassDefFoundError));
        }
    }
